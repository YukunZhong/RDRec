...
from model.templates_2stage import exp_templates, seq_templates, topn_templates, rea_templates, rev_templates
...
class RevSampler:
    ...
    def sample(self, num):
        ...
        for _ in range(num):
            ...
            idx = self.index_list[self.step]
            record = self.exp_data[idx]
            template = random.choice(rev_templates)
            inputs.append(template.format(record['user'],record['item']))

            outputs.append(record['review_summary'])
...
class AllBatchify:
    ...
    def __init__(self, exp_data, user2items_pos, negative_num, item_num, tokenizer, exp_len, batch_size, ratio):
        ...
        self.rev_sampler = RevSampler(exp_data)
        ...
        self.topn_num = int(ratio[3] / sum(ratio) * batch_size)
        self.rev_num = batch_size - self.exp_num - self.seq_num - self.rea_num - self.topn_num
        if self.exp_num > 0:
        ...
        elif self.topn_num > 0:
            self.batch_num = int(self.topn_sampler.sample_num / self.topn_num)
        else:
            self.batch_num = int(self.rev_sampler.sample_num / self.rev_num)
        self.batch_index = 0
    ...
    def next_batch(self):
        ...
        # rev
        tasks, inputs, outputs = self.rev_sampler.sample(self.rev_num)
        task_list.extend(tasks)
        input_list.extend(inputs)
        output_list.extend(outputs)
...
class RevBatchify:
    def __init__(self, exp_data, tokenizer, exp_len, batch_size):
        ...
        for x in exp_data:
            ...
            output_list.append(x['review_summary'])
