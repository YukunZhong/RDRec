...
class Solomon(T5ForConditionalGeneration):
    def __init__(self, config):
        super().__init__(config)
         # 只在 Solomon 里初始化一个 Conv1d，当作“短程注意力”
        ...
        if encoder_outputs is None:
            if task_id is None:
                ...
                input_emb, attention_mask = self.append_prompt(
                    task_id, input_ids, whole_word_ids, attention_mask
                )

            # --------- 在这里插入 Conv1d 层 ----------
            # emb: [B, S, D] -> [B, D, S] -> conv -> [B, D, S] -> [B, S, D]
            conv_in = input_emb.transpose(1,2)
            conv_out = self.conv1d(conv_in)       # (B, D, L)
            conv_out = self.conv_dropout(conv_out)
            conv_out = conv_out.transpose(1,2)    # (B, L, D)
            # 和原 embedding 相加，得到融合了 local context 的表示
            input_emb = input_emb + 0.2 * conv_out
            # ----------------------------------------
